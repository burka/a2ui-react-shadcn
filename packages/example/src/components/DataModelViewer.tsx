...
export function DataModelViewer({ messages }: DataModelViewerProps) {
  ...
  const surfaceStates = new Map<string, any>()
  ...
  messages.forEach((msg) => {
    if ('beginRendering' in msg) {
      ...
      values.forEach((dataValue) => {
        ...
        if (lastPart) {
          target[lastPart] = dataValue.value
        }
      })
  ...
  const renderValue = (value: any): React.ReactElement => {
    ...
    if (value === undefined) {
      return (
        <span className="text-[var(--color-text-tertiary)]">undefined</span>
      )
    }
      ...
      <div className="space-y-1">
        {entries.map(([key, value]) => {
          ...
          return (
            <div key={key}>
              <div
                className="flex items-center gap-2 py-0.5 px-2 hover:bg-[var(--color-bg-tertiary)] rounded cursor-pointer"
